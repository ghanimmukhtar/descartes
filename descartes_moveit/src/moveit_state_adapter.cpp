                                    ...
                                    const std::string &world_frame, const std::string &tcp_frame)
{
  ...
  primitive.dimensions[0] = 0.3;    primitive.dimensions[1] = 0.3;    primitive.dimensions[2] = 0.1;
  ...
  planning_scene_->setPlanningSceneDiffMsg(planning_scene_2);
  */
...
bool MoveitStateAdapter::getIK(const Eigen::Affine3d& pose, std::vector<double>& joint_pose) const
{
  ...
  if (robot_state_->setFromIK(joint_group_, tool_pose, tool_frame_))
  {
    ...
    //for (int i = 0; i < joint_pose.size(); i++)
      //  std::cout << "joint " << i << " is: " << joint_pose[i] << std::endl;
...
bool MoveitStateAdapter::getAllIK(const Eigen::Affine3d& pose, std::vector<std::vector<double> >& joint_poses) const
{
  ...
  joint_poses.clear();
  //std::cout << " there are: " << seed_states_.size() << " seeds, as follows: " << std::endl;
  for (size_t sample_iter = 0; sample_iter < seed_states_.size(); ++sample_iter)
  {
      /*std::cout << "seed no: " << sample_iter << " is: " << std::endl;
      for(size_t i = 0; i < seed_states_[sample_iter].size(); i++)
          std::cout << seed_states_[sample_iter][i] << std::endl;*/
...
bool MoveitStateAdapter::isInCollision(const std::vector<double>& joint_pose) const
{
  ...
  if (check_collisions_)
  {
    ...
    /*collision_detection::CollisionWorldConstPtr my_world = planning_scene_->getCollisionWorld();
